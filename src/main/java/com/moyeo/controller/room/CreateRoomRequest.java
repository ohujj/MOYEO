...
@Schema(description = """
        ...
        <ul>
          ...
          <li>PLACE_ONLY: 기본 정보, 나중에 장소를 정할 방식, 마감 시간</li>
          <li>SCHEDULE_AND_PLACE: 기본 정보, 일정 후보, 나중에 장소를 정할 방식, 마감 시간</li>
        </ul>
...
public record CreateRoomRequest(
        ...
        @Schema(
                description = """
                        ...
                        <ul>
                          ...
                          <li>PLACE_ONLY: 장소만 정하기. 장소는 나중에 정할 방식(RECOMMEND)만 저장하며, MIDDLE_POINT 또는 RANDOM 중 하나를 선택합니다.</li>
                          <li>SCHEDULE_AND_PLACE: 일정과 장소 둘 다 정하기. 일정 조율(VOTE)과 장소를 나중에 정할 방식(RECOMMEND)을 함께 저장합니다.</li>
                        </ul>
        ...
        @Schema(
                description = """
                        장소를 나중에 정할 방식입니다. planningType이 장소 정하기를 포함할 때 필수입니다.
        ...
        @Schema(description = "방장 출발지 주소입니다. placeRecommendationStrategy가 MIDDLE_POINT일 때만 필수입니다. 생성 시 방장 참여자 정보에 저장하며 실제 중간지점 계산은 이번 API 범위가 아닙니다.", example = "서울 강남구 테헤란로 123", maxLength = 255)
        @Size(max = 255)
        String hostDepartureAddress,

        @Schema(description = "생성 시점부터 마감까지 남은 시간(분)입니다. 서버가 deadlineAt을 계산합니다. 10분 단위, 최소 10분, 최대 72시간입니다.", example = "1440", minimum = "10", maximum = "4320")
        @Min(10)
        @Max(4320)
        int deadlineMinutes
) {
    ...
    @AssertTrue(message = "중간지점 추천에는 방장 출발지 주소가 필요합니다.")
    @Schema(hidden = true)
    public boolean isValidHostDepartureAddress() {
        if (!requiresPlace() || placeRecommendationStrategy != PlaceRecommendationStrategy.MIDDLE_POINT) {
            return true;
        }
        return hasText(hostDepartureAddress);
    }
